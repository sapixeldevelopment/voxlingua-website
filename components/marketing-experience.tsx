...
export function MarketingNavigation() {
  ...
  return <header className="site-header"><div className="site-container site-header-inner"><Link className="site-brand" href="/" aria-label="Dexlyy home"><span className="brand-mark" />Dexlyy<span className="site-brand-dot">.</span></Link><nav className="site-desktop-nav" aria-label="Main navigation"><a href="#product">Product</a><a href="#how-it-works">How it works</a><a href="#pricing">Pricing</a><Link href="/partners">Partners</Link><YouTubeLink /></nav><div className="site-header-actions"><Link href="/login" className="site-sign-in">Sign in</Link><Link href="#pricing" className="site-button site-button-green site-button-small">Get started <ArrowRight size={14} /></Link><a className="site-discord-join" href="https://discord.gg/kgzKr4bskN" target="_blank" rel="noopener noreferrer" aria-label="Join our Discord community"><img src="/discord-mark.svg" width="26" height="26" alt="" /><span><small>DISCORD COMMUNITY</small><strong>Join here</strong></span><ArrowRight size={16} /></a><button className="site-menu-button" type="button" aria-label={open ? "Close navigation" : "Open navigation"} aria-expanded={open} aria-controls="site-mobile-nav" onClick={() => setOpen(!open)}>{open ? <X size={22} /> : <Menu size={22} />}</button></div></div><nav className="site-mobile-nav" id="site-mobile-nav" aria-label="Mobile navigation" hidden={!open} onKeyDown={(event) => { if (event.key === "Escape") setOpen(false); }}>{[["Product", "/#product"], ["How it works", "/#how-it-works"], ["Pricing", "/#pricing"], ["Partners", "/partners"], ["Sign in", "/login"]].map(([label, href]) => <Link key={href} href={href} onClick={() => setOpen(false)}>{label}<ArrowRight size={15} /></Link>)}<a href="https://discord.gg/kgzKr4bskN" target="_blank" rel="noopener noreferrer" onClick={() => setOpen(false)}>Join our Discord<ArrowRight size={15} /></a><YouTubeLink /></nav></header>;
}

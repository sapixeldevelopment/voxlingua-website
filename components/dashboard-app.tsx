...
const NEEDS_REVIEW_STATUSES = ["pending", "under_review", "role_pending", "role_failed"];
...
export function ApplicationQueue({server, applications}: {server?: Server; applications: Application[]}) {
  ...
  const filteredApplications = useMemo(() => {
    ...
    }).sort((a,b) => order === "oldest" ? a.created_at.localeCompare(b.created_at) : b.created_at.localeCompare(a.created_at));
  }, [filter, scopedApplications, search, order]);
  ...
  useEffect(() => { setPage(1); }, [filter, search, order, server?.id]);
  ...
  return <section className="panel review-queue-panel">
    ...
    <label className="review-sort">Application order <select className="select" value={order} onChange={e=>setOrder(e.target.value)}><option value="oldest">Oldest application first</option><option value="newest">Newest application first</option></select></label>

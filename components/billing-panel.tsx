...
import { BILLING_INTERVALS, BILLING_PLANS, isGuidedPlan, planFeatures, PREPAID_PACKS, getPlanPrice, type BillingInterval, type OwnerBilling, type PlanKey, type PrepaidPackKey } from "@/lib/billing";
...
export default function BillingPanel({ onActivated, suppressCheckout = false }: { onActivated?: () => void; suppressCheckout?: boolean }) {
  ...
  const available = (active || paused) && billing?.plan_key !== "free";
  ...
  if (!data) return <section className="billing-panel billing-setup"><div className="billing-heading"><div><span className="eyebrow">billing setup</span><h2>Connect PayPal before taking payments</h2><p className="subtle">Add the PayPal client ID to the environment before opening checkout.</p></div><ShieldCheck size={26} /></div><p className="billing-hint">Use sandbox while testing. Keep PayPal secrets server-only and never place them in browser code.</p></section>;

  return <section className="billing-panel">
    {!billing?.paypal_subscription_id && <ReferralCodeField />}
    <div className="plan-feature-disclosure"><strong>Choose your interview experience</strong><p>Guided Voice: spoken questions and recorded answers. No transcripts, AI reviews, scores, voice analysis, or live AI conversation.</p><p>GPT Realtime: conversational interviews with recordings, transcripts, AI review, and voice analysis.</p></div>
    ...
    {available ? <>
      ...
      {!suppressCheckout && <div className="billing-current"><div><strong>Free Guided Voice · 5 interviews/month</strong><p>No card required. Recorded answers only; no transcripts or AI reviews.</p></div><button className="btn btn-primary" disabled={manageBusy || billing?.plan_key === "free"} onClick={()=>void (async()=>{setManageBusy(true);try{const response=await fetch("/api/billing/free",{method:"POST"});if(!response.ok)throw new Error("Free plan could not activate.");await load();onActivated?.();}catch{setMessage("Free plan could not activate. Please retry.");}finally{setManageBusy(false);}})()}>{billing?.plan_key === "free" ? "Free plan active" : "Start free"}</button></div>}
      ...
      <div className="billing-plan-grid">{(Object.keys(BILLING_PLANS) as PlanKey[]).filter(key => key !== "free").map(key => { const item = BILLING_PLANS[key]; const chosen = selectedPlan === key; const configured = Boolean(data.paypal.planIds[selectedInterval]?.[key]); return <button type="button" className={`billing-plan ${chosen ? "selected" : ""}`} key={key} onClick={() => setSelectedPlan(key)}><span className="billing-plan-top"><strong>{item.name}</strong><span>${getPlanPrice(key, selectedInterval)}<small>/{selectedInterval === "year" ? "year" : "month"}</small></span></span><span className="billing-plan-limit">{item.interviews === -1 ? "Unlimited" : item.interviews} interviews/month{item.dailyInterviews !== -1 ? ` · ${item.dailyInterviews}/day cap` : ""}</span><span className="billing-plan-copy">{planFeatures(key)}</span>{selectedInterval === "year" && <span className="billing-plan-savings">Save 2 months · ${(Number(item.yearlyPrice)/12).toFixed(2)}/mo equivalent</span>}{!configured && <span className="billing-plan-setup">Checkout setup pending</span>}<span className="billing-check">{chosen && <Check size={14} />}</span></button>; })}</div>
      {!suppressCheckout && <div className="billing-checkout"><div><strong>Start the {plan.name} plan</strong><p className="plan-feature-disclosure">{planFeatures(selectedPlan)}</p><p className="subtle">{plan.interviews === -1 ? "Unlimited" : plan.interviews} submitted interviews each month{plan.dailyInterviews !== -1 ? ` · ${plan.dailyInterviews} interviews/day cap` : ""} · {plan.servers} server{plan.servers === 1 ? "" : "s"} · {plan.staff === -1 ? "Unlimited" : plan.staff} staff</p><span className="billing-checkout-price">${getPlanPrice(selectedPlan, selectedInterval)} billed {selectedInterval === "year" ? "yearly · 2 months free" : "monthly"}</span></div>{selectedPlanId ? <PayPalButton mode="subscription" clientId={data.paypal.clientId} customId={data.paypal.customId} planId={selectedPlanId} planKey={selectedPlan} billingInterval={selectedInterval} onDone={refreshAfterPayment} /> : <div className="billing-checkout-pending">Add the PayPal {plan.name} {selectedInterval === "year" ? "yearly" : "monthly"} plan ID to enable checkout.</div>}</div>}
    </>}

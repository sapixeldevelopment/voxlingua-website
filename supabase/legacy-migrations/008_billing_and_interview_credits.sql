...
create policy "owners see their billing"
  on public.owner_billing for select
  to authenticated
  using (user_id = (select auth.uid()));

create policy "owners see their PayPal orders"
  on public.paypal_orders for select
  to authenticated
  using (user_id = (select auth.uid()));

create trigger owner_billing_touch before update on public.owner_billing
  for each row execute procedure public.touch_updated_at();
create trigger paypal_orders_touch before update on public.paypal_orders
  for each row execute procedure public.touch_updated_at();
...
create policy "subscribed owners create servers"
  ...
  with check (
    ...
    and exists (
      select 1 from public.owner_billing b
...
begin
  ...
  if v_billing.monthly_interview_limit = -1 then
    ...
    v_credit_source := 'prepaid';
    v_remaining := v_billing.prepaid_interviews;
  else

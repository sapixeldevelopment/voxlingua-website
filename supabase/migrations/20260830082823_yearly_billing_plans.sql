...
update public.owner_billing
   set subscription_period_end = coalesce(subscription_period_end, monthly_period_end);

alter table public.owner_billing
  alter column subscription_period_end set not null;

alter table public.owner_billing
  drop constraint if exists owner_billing_billing_interval_check;

alter table public.owner_billing
  add constraint owner_billing_billing_interval_check

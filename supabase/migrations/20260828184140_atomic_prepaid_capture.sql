...
as $$
declare v_order record; v_billing record; v_balance integer;
begin
  ...
  if v_order.status = 'completed' then return jsonb_build_object('ok', true, 'credits', v_order.interview_credits, 'already_completed', true); end if;
  ...
  update public.owner_billing set prepaid_interviews = prepaid_interviews + v_order.interview_credits where user_id = v_order.user_id returning prepaid_interviews into v_balance;
  update public.paypal_orders set status = 'completed', paypal_capture_id = p_capture_id where id = p_order_id;

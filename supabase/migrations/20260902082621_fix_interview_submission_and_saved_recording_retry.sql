-- Fix SQL conditional expressions in the existing atomic submission function.
-- Keep ownership, row locking, billing and completed-session idempotency intact.
...
begin
  ...
  if p_transcript is null or pg_catalog.jsonb_typeof(p_transcript) is distinct from 'array'
  ...
  v_question_count := greatest(1, least(8, coalesce(nullif(v_question_count, 0), 3)));
  v_required_turns := greatest(1, pg_catalog.ceil(v_question_count * 0.6)::integer);
  ...
  if v_substantive_turns < v_required_turns
     or v_total_words < greatest(12, v_question_count * 4)
     or v_total_chars < greatest(60, v_question_count * 18) then
    raise exception 'Please give clear spoken answers to the interview questions before submitting.' using errcode = 'P0001';
  end if;
  -- Review/retries do not extend recording time. Use Storage's server-controlled
  -- upload timestamp, not the time the applicant retries a failed submission.
  select o.updated_at into v_recording_uploaded_at
    from storage.objects o
   where o.bucket_id = 'interview-recordings'
     and o.name = v_session.recording_path
     and o.name = v_session.server_id::text || '/' || v_session.id::text || '/recording.webm';
  if v_recording_uploaded_at is null or v_recording_uploaded_at < v_session.started_at then
    raise exception 'The interview recording must finish uploading before submission.' using errcode = 'P0001';
  end if;
  if pg_catalog.date_part('epoch', v_recording_uploaded_at - v_session.started_at) > v_time_limit_seconds + 120 then

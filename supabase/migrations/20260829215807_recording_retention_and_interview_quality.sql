...
begin
  if not exists (
    ...
    where conname = 'applications_reviewer_role_check'
      and conrelid = 'public.applications'::regclass
  ) then
    alter table public.applications
      add constraint applications_reviewer_role_check
      check (reviewer_role is null or reviewer_role in ('owner', 'admin', 'reviewer'));
  end if;
end
$$;

alter table public.servers
  drop constraint if exists servers_retention_covers_cooldown_check;

create index if not exists interview_sessions_recording_cleanup_idx
  on public.interview_sessions (server_id, completed_at)
  where recording_path is not null;

comment on column public.servers.application_retention_days is
  'Days to retain submitted interview audio. Zero keeps recordings until manually deleted. Application history and transcripts are retained until manually deleted.';
comment on column public.interview_sessions.recording_deleted_at is
  'When the private audio recording was deleted by the configured retention policy.';
comment on column public.applications.reviewer_role is
  'The reviewer server role captured when the final decision was made.';

-- This signed-in RPC is the final billing and submission boundary. Keep the
-- quality and duration checks here so direct Supabase calls cannot bypass the
-- equivalent checks in the Next.js API route.
...
begin
  ...
  select i.id, i.status, i.application_id, i.server_id, i.started_at,
         i.recording_path, a.applicant_user_id, s.owner_id
    into v_session
  ...
  if v_session.started_at is null then
    raise exception 'The interview was not started correctly. Please start over.' using errcode = 'P0001';
  end if;
  ...
  v_question_count := pg_catalog.greatest(1, pg_catalog.least(20, coalesce(nullif(v_question_count, 0), 3)));
  v_required_turns := pg_catalog.greatest(1, pg_catalog.ceil(v_question_count * 0.6)::integer);
  v_time_limit_seconds := pg_catalog.least(1200, pg_catalog.greatest(480, 120 + v_question_count * 120));
  ...
  if pg_catalog.date_part('epoch', pg_catalog.now() - v_session.started_at) > v_time_limit_seconds + 120 then
    raise exception 'This interview attempt exceeded its time limit. Please start over and try again.' using errcode = 'P0001';
  end if;
